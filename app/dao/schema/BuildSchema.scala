...
class BuildSchema(tag: Tag) extends Table[SuccessfulBuild](tag, "SUCCESSFUL_BUILDS") {
    def buildId = column[String]("BUILD_ID", O.PrimaryKey) // This is the primary key column
    ...
    def projectPath = column[String]("PROJECT_PATH")

    // Every table needs a * projection with the same type as the table's type parameter
    def * = (buildId, startDate, endDate, projectName, projectUrl, projectPath) <> (SuccessfulBuild.tupled, SuccessfulBuild.unapply)
}
